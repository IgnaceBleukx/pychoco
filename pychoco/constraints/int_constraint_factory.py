...
class IntConstraintFactory(ABC):
    ...
    def cumulative_impl(self, bv: BoolVar, start: List[IntVar], dur: List[IntVar], end: List[IntVar], heights: List[IntVar], capacity: IntVar, incremental: bool = True):
            """
            Creates a half-reified cumulative constraint: Enforces that at each point in time,
            the cumulated height of the set of tasks that overlap that point
            does not exceed a given limit if the boolean variable is set to True.
            ...
            # tasks_handle = make_intvar_array(tasks)
            vars_handle = make_intvar_array(heights)
            backend.cumulative_impl(self.handle, bv.handle, start_handle, dur_handle, end_handle, vars_handle, capacity.handle, incremental)
